...
\end{lemma}
\begin{proof}
Fix a nonnegative test function \(\phi\in C_c^\infty(I)\). By the phase--velocity identity,
\[
 \int\!\phi\,\Big(\Im\frac{\xi'}{\xi}-\Im\frac{\dettwo'}{\dettwo}+\mathsf H[u']\Big)\Big(\tfrac12+it\Big)dt
 \ =\ \int_{\{\Re s>1/2\}} P_{s}[\phi] \ d\mu(s)\ \ge 0.
\]
The left side equals \(\int_I \phi(t)\,(-w'(t))\,dt\) with \(w\) the normalized phase mismatch. Since \(\|P_{s}[\phi]\|_{L^\infty}\le 1\) and the Poisson kernel has unit \(t\)–mass, the hypothesis yields
\[
 \int_I \phi(t)\,(-w'(t))\,dt\ \le\ \mu(Q(I))\ \le\ \pi/2.
\]
Approximating the indicator of \(I\) by such \(\phi\) gives \(\int_I (-w')\le \pi/2\). By the phase-cone criterion, \(w(t)\in[-\pi/2,\pi/2]\) a.e. on \(I\), hence \(\Re(2\mathcal J(\tfrac12+it))\ge 0\) a.e. on \(I\).
\end{proof}
