Kato projector continuity/stability (A5) — missing/uncertain mathlib facts

1) Jacobi/derivative identity at eigenvalue (naming): need a lemma equating
   (Polynomial.derivative (Matrix.charpoly A)).eval λ with trace (adj (λ•I - A)).
   Referenced as `Matrix.trace_adj_deriv_det_sub_left_eval` in notes; confirm/expose the exact mathlib name.

2) Rank‑one adjugate square identity: for M = (λ•I − A) with simple eigenvalue λ,
   show (adj M)^2 = (trace (adj M)) • (adj M) as linear maps/matrices to prove idempotence of
   P = (trace (adj M))⁻¹ • adj M. A direct lemma or a standard rank‑one outer‑product argument would suffice.

3) IFT for F(A,z) = det(z•I − A) near (A, λ): out‑of‑the‑box application that returns a unique
   analytic branch z(A') with z(A) = λ, using (∂/∂z)F ≠ 0 and a spectral gap. Provide a usable statement
   to pick λₙ and show simplicity persists.

4) Dimension bridge: at a simple eigenvalue, `range (adj (λ•I − A)) = ker (λ•I − A)` proved via
   `range ⊆ ker`, nontriviality of adjugate (trace ≠ 0), and `finrank ker = 1`.
...
- MATH-BLOCKER: Kato simple-eigenvalue branch via IFT and projector continuity
  - Location: `ym/Kato/SimpleEigenStability.lean`
  - Lean goal / statement: Implement `continuousAt_katoProj` and projector convergence near an isolated simple eigenvalue using the implicit function theorem for `F(A,z)=det(zI−A)` with `∂F/∂z ≠ 0` at `(A,λ₀)`, and identify the rank‑one projector via `adj(λ•I−A)` with a local quantitative bound.
  - Proposed approach: Use mathlib’s IFT (`Mathlib/Analysis/Calculus/ImplicitFunction`) to construct the branch `z(A')`, continuity of adjugate and trace to show continuity of `katoProj`, and the Jacobi formula (`trace (adj (λI - A)) = ∂_λ det(λI - A)`) to ensure nonvanishing denominator at a simple root. Combine to give `‖P(A',z(A')) - P(A,λ₀)‖ ≤ C (‖A' - A‖ + |z(A') - λ₀|)` locally. These prerequisites are substantial to verify fully here; record as blocker and keep interface skeletons.
